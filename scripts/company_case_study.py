#!/usr/bin/env python3
"""
Company Case Study: Extract all crypto-related snippets from saved filings.
Generalized script that accepts company parameters via command line.

Usage:
    python scripts/company_case_study.py --cik 0001633917 --name "PayPal"
    python scripts/company_case_study.py --cik 0000886982 --name "Goldman Sachs"
    python scripts/company_case_study.py --cik 0001318605 --name "Tesla"
    python scripts/company_case_study.py --cik 0001045810 --name "NVIDIA"
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="Extract crypto-related snippets from SEC filings for a specific company."
    )
    parser.add_argument(
        "--cik",
        required=True,
        help="10-digit CIK (e.g., 0001633917)",
    )
    parser.add_argument(
        "--name",
        required=True,
        help="Company name (e.g., 'PayPal')",
    )
    args = parser.parse_args()

    cik = args.cik
    company_name = args.name
    output_filename = f"{sanitize_filename(company_name)}_crypto_snippets.xlsx"
    output_file = DATA_DIR / output_filename

    print(f"{company_name} Case Study: Extracting all crypto snippets")
    print("=" * 70)

    # Find company folder
    company_folder = CLOUD_FOLDER / cik

    if not company_folder.exists():
        print(f"Error: Company folder not found at {company_folder}")
        return

    print(f"\nScanning folder: {company_folder}")

    # Get all text files
    files = []
    for ext in [".txt", ".htm", ".html"]:
        files.extend(company_folder.glob(f"*{ext}"))
